...
Phormat   		db "%08X-%08X-%08X-%08X",0
NoMail			db "insert ur mail",0
TooLong			db "too long.",0
Mail8chr		db "min 8 chars plz",0
NoAront			db "u forgot the @ char",0

.code
GenKey proc hWin:DWORD

		invoke GetDlgItemText,hWin,IDC_MAIL,offset NameBuffer,400h
		.if eax == 0
			invoke SetDlgItemText,hWin,IDC_SERIAL,offset NoMail
			ret
		.elseif eax < 8
			invoke SetDlgItemText,hWin,IDC_SERIAL,offset Mail8chr
			ret
		.elseif eax > 40
		...
		invoke Ismail,offset NameBuffer,400h
		.if eax == 0
			invoke SetDlgItemText,hWin,IDC_SERIAL,offset NoAront
			ret
		.endif
		...
		invoke lstrcpy,offset SerialBuffer,offset NameBuffer
		
		mov ScndRndval,11E1A91Ah
		invoke Rndproc,7FFFFFFFh
		mov ThirdRndval, eax
		
		lea edi, FirstRndval
		lea eax, SerialBuffer
		call HexSwap
		...
		mov ecx, FirstRndval
		call CustomRndProc
		mov FourthRndval, eax
		pop ebp
		
		invoke wsprintf,offset FinalBuffer,offset Phormat,FirstRndval,ScndRndval,ThirdRndval,FourthRndval
